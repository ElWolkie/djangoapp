...
class CuotaCobroCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not id_inscripcion or not nombre_cuota or monto is None:
            return Response({
                'success': False,
                'message': 'Faltan parámetros obligatorios: idInscripcion, nombreCuota, monto'
            }, status=status.HTTP_400_BAD_REQUEST)

        try:
            inscripcion = Inscripcion.objects.select_related('idCohorte__idFormacion', 'idPersona').filter(
                idInscripcion=id_inscripcion, is_active=True
            ).first()
            if not inscripcion:
                return Response({'success': False, 'message': 'Inscripción no encontrada o inactiva.'},
                                status=status.HTTP_404_NOT_FOUND)

            # bloquear inscripcion para evitar race conditions
            # (opcional, dependiendo de concurrencia)
            inscripcion = Inscripcion.objects.select_for_update().get(pk=inscripcion.pk)

            # 1) Validar configuracion y tasa
            configuracion = Configuracion.objects.first()
            if not configuracion:
                return Response({'success': False, 'message': 'No hay configuración activa.'},
                                status=status.HTTP_400_BAD_REQUEST)

            moneda_config = getattr(configuracion, 'moneda', None)
            tasa = Tasa.objects.filter(idMoneda=moneda_config).order_by('-idTasa').first()
            if not tasa:
                return Response({'success': False, 'message': 'No hay tasa configurada para la moneda.'},
                                status=status.HTTP_400_BAD_REQUEST)
            ...
            if not periodo_activo:
                periodo_activo = periodoContable.objects.order_by('-idPeriodo').first()
            if not periodo_activo:
                return Response({'success': False, 'message': 'No hay periodo contable activo.'},
                                status=status.HTTP_400_BAD_REQUEST)

            # 3) Buscar la InscripcionCuota EN ESPERA
            # Usamos icase para evitar problemas de mayúsculas/minúsculas
            try:
                ...
                return Response({'success': False, 'message': 'La cuota no está disponible para pago (no existe o no está EN ESPERA).'},
                                status=status.HTTP_400_BAD_REQUEST)
            except InscripcionCuota.MultipleObjectsReturned:
                return Response({'success': False, 'message': 'Duplicidad de cuotas. Contacte soporte.'},
                                status=status.HTTP_400_BAD_REQUEST)

            # 4) Ajustar/validar monto con valor real
            ...
            if valor_real is not None:
                ...
                if monto_decimal != valor_real_dec:
                    # Forzamos el monto al valor real
            ...
            # 5) Crear asiento provisorio (CUOTA)
            numero_asiento = f"CUOTA-{now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
            asiento = AsientoContable.objects.create(
                numeroAsiento=numero_asiento,
                fechaAsiento=now().date(),
                conceptoAsiento=f"Asiento CUOTA {cuota.idCuota.nombreCuota} - Inscripción {inscripcion.idInscripcion}",
                idPeriodo=periodo_activo
            )

            # 6) Crear Nota de cobro (tipo CUOTA)
            numero_nota = generar_numero_nota()
            nota = Nota.objects.create(
                idAsiento=asiento,
                ...
                totalNota=monto_decimal,
                idTasa=tasa,
                estado='PENDIENTE',
                observaciones=f"Nota CUOTA generada automáticamente para {cuota.idCuota.nombreCuota}"
            )

            # 7) Relacionar la nota con la inscripción y la cuota
            NotaRelacionada.objects.create(idNota=nota, idInscripcion=inscripcion, idCuota=cuota)

            # 8) Crear PagoTemporal
            ...
            # 9) Crear DetalleAsiento: localizar PlanArticulo para CUOTA
            # Manejo robusto si PlanArticulo.tipo es BooleanField ó int (0/1)
            ...
            if not plan_debe or not plan_haber:
                # No debe bloquear la creación de nota/pago temporal, pero avisamos
                logger.warning("No se encontraron PlanArticulo para CUOTA (debe/haber). Revisar configuración contable.")
            else:
                # crear los dos detalles del asiento
                DetalleAsiento.objects.create(
                    idAsiento=asiento,
                    idPlanCuenta=plan_debe.idPlanCuenta,
                    debe=float(monto_decimal),
                    haber=Decimal('0.00')
                )
                DetalleAsiento.objects.create(
                    idAsiento=asiento,
                    ...
                    debe=Decimal('0.00'),
                    haber=float(monto_decimal)
                )

            # 10) Marcar InscripcionCuota como PENDIENTE (no modificar montoPagado aún)
            cuota.estadoPago = 'PENDIENTE'
            cuota.save(update_fields=['estadoPago'])

            # 11) Marcar inscripcion.estadoPago como PARCIAL o mantener PENDIENTE según lógica
            # (no sumamos montoPagado todavía hasta confirmación)
            # Si deseas cambiar inscripcion.estadoPago a 'PARCIAL' si hay pagos parciales:
