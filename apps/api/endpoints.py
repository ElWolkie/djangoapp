    ...
    def post(self, request):
        ...
        try:
            ...
            for f in required_fields:
                if f not in data:
                    return Response({'success': False, 'message': f'Campo requerido faltante: {f}'}, status=status.HTTP_400_BAD_REQUEST)

            try:
                nota = Nota.objects.select_related('idPersona').filter(idNota=data['idNota']).first()
                if not nota:
                    return Response({'success': False, 'message': 'Nota no encontrada'}, status=status.HTTP_404_NOT_FOUND)
                logger.debug('✅ Nota encontrada: %s', nota.numeroNota)
            except Exception:
                return Response({'success': False, 'message': 'Error buscando la nota'}, status=status.HTTP_400_BAD_REQUEST)

            if nota.estado == 'PAGADA':
                return Response({'success': False, 'message': 'Esta nota ya ha sido pagada completamente'}, status=status.HTTP_400_BAD_REQUEST)

            try:
                monto_pago = Decimal(str(data['monto']))
                if monto_pago <= 0:
                    return Response({'success': False, 'message': 'El monto debe ser mayor a 0'}, status=status.HTTP_400_BAD_REQUEST)
                if monto_pago > nota.totalNota:
                    return Response({'success': False, 'message': f'El monto no puede ser mayor al total de la nota (${nota.totalNota})'}, status=status.HTTP_400_BAD_REQUEST)
            except (ValueError, InvalidOperation):
                return Response({'success': False, 'message': 'Formato de monto inválido'}, status=status.HTTP_400_BAD_REQUEST)

            configuracion = Configuracion.objects.first()
            if not configuracion:
                return Response({'success': False, 'message': 'Configuración del sistema no encontrada'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            asiento = AsientoContable.objects.create(numeroAsiento=numero_asiento, fechaAsiento=now().date(), conceptoAsiento=f"Pago de {data['formaPago']} - Nota: {nota.numeroNota}", idPeriodo=periodo_activo)

            pago = Pago.objects.create(idNota=nota, idAsiento=asiento, idTasa=tasa, monto=monto_pago, fechaPago=data['fechaPago'], formaPago=data['formaPago'], referencia=data.get('referencia', ''), observaciones=data.get('observaciones', ''))
            ...
            try:
                ...
                if nota_relacionada and getattr(nota_relacionada, 'idInscripcion', None):
                    inscripcion = nota_relacionada.idInscripcion
                    inscripcion.estadoPago = 'PAGADO' if monto_pago >= nota.totalNota else 'PARCIAL'
                    inscripcion.montoPagado = (inscripcion.montoPagado or Decimal('0')) + monto_pago
                    inscripcion.save()
            except Exception:
                logger.exception('No se pudo actualizar inscripción relacionada (no crítico)')

            try:
                plan_articulo_debe = PlanArticulo.objects.filter(tipoArticulo=nota.tipoArticulo, tipo=True).order_by('-fecha').first()
                plan_articulo_haber = PlanArticulo.objects.filter(tipoArticulo=nota.tipoArticulo, tipo=False).order_by('-fecha').first()
                if plan_articulo_debe and plan_articulo_haber:
                    DetalleAsiento.objects.create(idAsiento=asiento, idPlanCuenta=plan_articulo_debe.idPlanCuenta, debe=monto_pago, haber=Decimal('0.00'))
                    DetalleAsiento.objects.create(idAsiento=asiento, idPlanCuenta=plan_articulo_haber.idPlanCuenta, debe=Decimal('0.00'), haber=monto_pago)
                else:
                    logger.warning('No se encontraron cuentas contables para %s', nota.tipoArticulo)
            except Exception:
                logger.exception('Error creando detalles de asiento (no crítico)')

            response_data = {
                'success': True,
                'message': '¡Pago procesado exitosamente! 🎉',
                'data': {'idPago': pago.idPago, 'numeroAsiento': asiento.numeroAsiento, 'monto': float(pago.monto), 'fechaPago': pago.fechaPago.isoformat(), 'formaPago': pago.formaPago, 'referencia': pago.referencia or '', 'nota': {'idNota': nota.idNota, 'numeroNota': nota.numeroNota, 'nuevoEstado': nota.estado, 'totalNota': float(nota.totalNota)}}
            }
