...
interface Cargo {
  idCargo: number;
  nombreCargo: string;
  estadoCargo: 'ACTIVO' | 'INACTIVO' | string;
  fechaCargo: string;
}

export default function PantallaCargos() {
  const [cargos, setCargos] = useState<Cargo[]>([]);
  const [mostradas, setMostradas] = useState<Cargo[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get<Cargo[]>('/api/cargo/');
        const data = res.data ?? [];
        setCargos(data);
        setMostradas(data);
      } catch (err: any) {
  ...
  useEffect(() => {
    const fil = cargos.filter(c =>
      c.nombreCargo?.toLowerCase().includes(searchText.toLowerCase()) ||
      (c.estadoCargo ?? '').toLowerCase().includes(searchText.toLowerCase())
    );
    setMostradas(fil);
  }, [searchText, cargos]);
  ...
  const openModal = (c: Cargo) => {
    setSelected(c);
    setModalVisible(true);
  };

  const renderItem = ({ item, index }: { item: Cargo; index: number }) => {
    ...
    return (
      <Animated.View style={[
        ...
        <View style={styles.header}>
          <Text style={styles.name}>{item.nombreCargo}</Text>
          <View style={[
            styles.badge,
            item.estadoCargo === 'ACTIVO' ? styles.badgeActive : styles.badgeInactive
          ]}>
            <Text style={styles.badgeText}>
              {item.estadoCargo === 'ACTIVO' ? 'Activo' : 'Inactivo'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.detailText}>Registrado: {item.fechaCargo}</Text>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={mostradas}
        keyExtractor={c => c.idCargo.toString()}
      ...
      >
        <View style={styles.modalContent}>
          <ScrollView>
            <Text style={styles.modalTitle}>{selected?.nombreCargo}</Text>
            {selected && ([
              ['ID', String(selected.idCargo)],
              ['Nombre', selected.nombreCargo],
              ['Estado', selected.estadoCargo],
              ['Fecha', selected.fechaCargo]
